...
/**
 * Demo-only direct Anthropic integration.
 *
...
class AnthropicAiGateway(
    ...
    private val fastModel: String = BuildConfig.ANTHROPIC_FAST_MODEL,
    private val reportModel: String = BuildConfig.ANTHROPIC_REPORT_MODEL,
    ...
    private fun parseCard(raw: String): AiCard {
        ...
        if (question.isBlank() || alternative.isBlank()) {
            throw AnthropicApiException("Card JSON is incomplete")
        }
    ...
    private fun extractJson(raw: String): JSONObject {
        ...
        if (start < 0 || end <= start) throw AnthropicApiException("No JSON object in response")
